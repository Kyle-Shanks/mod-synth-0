...
export function Knob({
  ...
  const [macroMenu, setMacroMenu] = useState<{ x: number; y: number } | null>(
    null,
  )
    ...
    (e: React.PointerEvent) => {
      ...
      if (isLog) {
        ...
        newValue = Math.max(
          ...
          Math.min(
            max,
            dragRef.current.currentValue + dy * range * sensitivity,
          ),
        )
      }
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        className={classes(
          styles.knobSvg,
          isMacroExposed && styles.knobSvgLocked,
        )}
      ...
      <div
        className={classes(
          styles.labelArea,
          isMacroExposed && styles.labelAreaLocked,
        )}
      >
        <span
