...
import { restoreSavedPatch, setupAutosave, clearPatchStorage } from './persistence/storage'
import { serializePatch, deserializePatch, validatePatchJson } from './persistence/serialization'
...
export default function App() {
  ...
  const handleNewPatch = useCallback(() => {
    ...
    if (!window.confirm('clear current patch? unsaved changes will be lost.')) return
    clearPatch()
    clearPatchStorage()
  }, [clearPatch])
      ...
      {!started ? (
        ...
        <>
          ...
          >
            ...
            {/* separator dot */}
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--shade2)' }}>·</span>

            {/* new */}
            <button onClick={handleNewPatch} style={topBarBtnStyle} title="new patch">
              new
            </button>
            ...
            {/* export */}
            <button onClick={handleExport} style={topBarBtnStyle} title="export patch as json">
              export
            </button>

            {/* import */}
            <button onClick={() => fileInputRef.current?.click()} style={topBarBtnStyle} title="import patch from json">
              import
            </button>
            <input
              ref={fileInputRef}
