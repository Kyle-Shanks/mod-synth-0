...
export const EnvFollowDefinition: ModuleDefinition<
  ...
  {
    attack: { type: 'float'; min: 0.001; max: 0.5; default: 0.01; label: 'atk'; unit: 's' }
    release: { type: 'float'; min: 0.001; max: 2; default: 0.1; label: 'rel'; unit: 's' }
  },
  EnvFollowState
> = {
  id: 'envfollow',
  ...
  process(inputs, outputs, params, state, context) {
    const atkCoeff = 1 - Math.exp(-1 / Math.max(1, params.attack  * context.sampleRate))
    const relCoeff = 1 - Math.exp(-1 / Math.max(1, params.release * context.sampleRate))
    for (let i = 0; i < 128; i++) {
